          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="grid gap-4 md:grid-cols-2">
              {Object.entries(MARKET_COMPARISON).map(([key, profile]) => {
                ...
                return (
                  <Panel key={key} className={isActive ? 'border-[var(--line)] bg-[linear-gradient(180deg,rgba(181,35,50,0.16),rgba(255,255,255,0.03))]' : ''}>
              ...
              return (
                <motion.div key={key} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }}>
                  <Panel className="h-full">
                    ...
                    <div className="mt-5 rounded-2xl border border-[var(--line)] bg-[rgba(181,35,50,0.14)] p-4 text-sm leading-6 text-[var(--text-soft)]">
