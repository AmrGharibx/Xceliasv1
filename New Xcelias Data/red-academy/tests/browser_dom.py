...
with tempfile.TemporaryDirectory(prefix='red-private-browser-') as temporary:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            save(page); check('Batch and ten-session schedule created through form',len(state(owner)['batches'][0]['session_dates'])==10)
            ...
            save(page)
            data=state(owner); check('Enrollment creates independent empty checklist',len(data['trainees'])==1 and len(data['attendance_10day'])==1 and not any(data['attendance_10day'][0]['days']))
            ...
            go(page,'summaries');page.locator('[data-checklist][data-day="0"]').check()
            page.wait_for_function('__redTest.store.data.attendance_10day[0].days[0]===true')
            check('Independent checklist toggle persists',state(owner)['attendance_10day'][0]['days'][0])
            ...
            for key,value in [('mapping',5),('product_knowledge',5),('presentability',5),('soft_skills',4.5)]:
            ...
            check('Four skill inputs recalculate score and suggested outcome live')
