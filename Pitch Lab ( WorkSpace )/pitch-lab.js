...
function applyLang() {
  ...
  if (chatInput) chatInput.placeholder = lang === "ar" ? "اكتب ردّك..." : "Type your response...";
}
...
async function generateAIScenario(difficulty) {
  ...
  try {
    // Request 4096 tokens to ensure the full scenario JSON is never truncated.
...
function renderScenarios() {
  ...
  const cards = SCENARIOS.map((s) => {
    ...
    return `<div class="scenario-card${s.isGenerated ? " scenario-ai-card" : ""}" ${styleAttr} data-id="${escHtml(s.id)}" data-difficulty="${escHtml(s.difficulty)}" role="button" tabindex="0" aria-label="${name}">
      ${aiBadge}
...
function applyDifficultyFilter() {
  const grid = $("scenarios-grid");
  if (!grid) return;
  grid.querySelectorAll(".scenario-card[data-difficulty]").forEach((card) => {
    const show = activeFilter === "all" || card.dataset.difficulty === activeFilter;
    card.style.display = show ? "" : "none";
  });
  document.querySelectorAll(".diff-filter-btn").forEach((btn) => {
    btn.classList.toggle("active", btn.dataset.diff === activeFilter);
  });
}
...
function startCall() {
  ...
  if (_openLine) {
    const _line = lang === 'ar' ? (_openLine.ar || _openLine.en) : (_openLine.en || _openLine.ar);
    setTimeout(() => {
      if (!currentScenario) return; // call was ended
      appendBubble('client', _line);
      // Gemini API requires contents to start with a user turn.
      // Prepend a silent synthetic user marker so the first item is always 'user'.
      callMessages.push({ role: 'user', content: '.' });
      callMessages.push({ role: 'assistant', content: _line });
      callTranscript.push({ speaker: 'Client', text: _line });
      setSendDisabled(false); // ensure button is re-enabled after opening line
...
async function sendMessage() {
  ...
  try {
    ...
    // Language instruction — driven by the UI toggle
    ...
    const ENG_LANG = "English — conversational and natural. You are the Egyptian character described above; stay true to your personality completely. Any Arabic phrases or dialect expressions your Personality section specifies (يلا بلا, يعني, طب, etc.) are still part of who you are and may appear when authentically in-character, but your primary language is English. Respond exactly as this person would on a real phone call in English — not robotic, not translated, just natural.";
    const sysPrompt = currentScenario.systemPrompt(lang === "ar" ? EGY_LANG : ENG_LANG) + "\n\n" + MARKET_KNOWLEDGE;
...
async function endCall() {
  ...
  try {
    ...
    // Request 4096 tokens so the full structured debrief JSON is never truncated.
    const rawJson = await callGemini(
      null,
 ...
 * @param {{maxOutputTokens?:number}|null} [generationConfig]
 */
async function callGemini(systemPrompt, messages, generationConfig = null) {
  const body = { messages };
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".diff-filter-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      activeFilter = btn.dataset.diff || "all";
      applyDifficultyFilter();
    });
  });
  ...
  if (btnExport) btnExport.addEventListener("click", () => {
    ...
    const scenarioName = scenario?.name?.en || scenario?.name || 'Unknown';
    lines.push(`Pitch Lab — Call Transcript`);
    ...
    a.href = url;
    a.download = `transcript-${(scenario?.id || 'call').replace(/[^a-z0-9-]/gi, '_')}-${Date.now()}.txt`;
