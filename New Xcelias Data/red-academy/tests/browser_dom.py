...
with tempfile.TemporaryDirectory(prefix='red-private-browser-') as temporary:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            click(page,'theme')
            check('Light mode shows the academy logo and signature while the bottom promo is hidden and reports stay reachable',page.evaluate('document.body.classList.contains("light") && document.querySelector(".academy-brand-logo")?.alt==="Red Training Academy" && getComputedStyle(document.querySelector(".academy-brand-logo")).display==="block" && document.querySelector(".academy-brand-signature")?.textContent.trim()==="X Gh" && Number.parseFloat(getComputedStyle(document.querySelector(".academy-brand-signature")).fontSize)>=16 && getComputedStyle(document.querySelector(".sidebar-note")).display==="none" && Boolean(Array.from(document.querySelectorAll(".nav-link")).some(link=>link.getAttribute("href")==="#/analytics")) && getComputedStyle(document.querySelector("canvas.starfield")).mixBlendMode==="multiply" && getComputedStyle(document.querySelector(".sidebar")).backdropFilter.includes("blur")'))
            if args.screenshots:
