...
export function helpModal(ctx){
 ...
 const guide=document.querySelector('.modal-body .profile-notes'),paragraphs=guide?.querySelectorAll('p');
 if(paragraphs?.[1])paragraphs[1].textContent='Choose the batch date range and schedule the actual number of sessions needed, from 1 to 366. Imported historical batches retain missing or irregular dates instead of inventing a schedule.';
 if(paragraphs?.[2])paragraphs[2].textContent='Each trainee’s live session checklist follows the batch schedule and updates from Daily Attendance; no separate completion checkbox is needed. Historical 10-day snapshots remain archived.';
 if(paragraphs?.[3])paragraphs[3].textContent='Use Daily attendance for entry and Attendance records for the complete source register. Present and Tour Day count as attended; tour no-shows are marked Absent. Unrecorded days are not absences. The manual late flag and arrival-based lateness remain separate.';
