...
async function quickAttendance(traineeId,checkout=false){const t=store.data.trainees.find(t=>t.id===traineeId);if(!t)return;if(t.enrollment_status==='Stopped Attending')throw new Error('This trainee is marked as stopped attending. Change the enrollment status before recording new attendance.');if(ctx.date!==today()){attendanceForm(ctx,t);return;}const record=store.data.daily_attendance.find(r=>r.trainee_id===t.id&&r.date===ctx.date),now=new Date().toISOString();if(cairoDate(now)!==ctx.date)throw new Error('The date changed. Select the current Cairo date and try again.');if(checkout&&(!record?.arrival_time||now<record.arrival_time))throw new Error('Departure must be after the recorded arrival. Use the time editor to correct the entry.');const data={trainee_id:t.id,batch_id:t.batch_id,date:ctx.date,status:'Present',arrival_time:checkout?record.arrival_time:now,departure_time:checkout?now:null,is_late:record?.is_late||false,assessment_day:record?.assessment_day||false};await store.mutate('daily_attendance',record?'update':'create',record,data);toast(`${t.trainee_name} checked ${checkout?'out':'in'}.`);}
async function bulkAttendance(){if(!ctx.selection.size)return;const selectedStatus=document.getElementById('bulk-status')?.value||'Present',assessmentDay=selectedStatus==='Assessment Day',status=assessmentDay?'Present':selectedStatus;if(!STATUSES.includes(status))throw new Error('Choose a valid status.');const entries=[...ctx.selection].map(traineeId=>{const t=store.data.trainees.find(t=>t.id===traineeId);if(!t||t.batch_id!==ctx.batchId||t.enrollment_status==='Stopped Attending')throw new Error('The selected roster changed. Select active trainees again.');const old=store.data.daily_attendance.find(r=>r.trainee_id===t.id&&r.date===ctx.date),skip=['Absent','Off Day'].includes(status);return {action:old?'update':'create',id:old?.id,expectedVersion:old?.version,data:{trainee_id:t.id,batch_id:t.batch_id,date:ctx.date,status,assessment_day:assessmentDay,arrival_time:skip?null:old?.arrival_time||null,departure_time:skip?null:old?.departure_time||null,is_late:skip?false:old?.is_late||false}};});await store.write({table:'daily_attendance',records:entries});ctx.selection.clear();render();toast(`${entries.length} attendance records saved as ${selectedStatus.toLowerCase()}.`);}
function deleteOperationalRecord(table,record){if(!store.canDelete(table)||!record||record.source_id)throw new Error('Your role cannot delete this record, or it is protected imported data.');const batch=table==='batches',title=batch?record.batch_name:record.trainee_name,description=batch?'This permanently removes the batch, its enrollments, attendance and checklists. Assessment revisions remain in history; imported source pages are preserved.':'This permanently removes the enrollment, attendance, checklists and assessment. Assessment revisions remain in history; imported source pages are preserved.';confirmDialog(`Delete ${title}?`,description,async()=>{if(batch){if(ctx.batchId===record.id)ctx.batchId='';ctx.detailId='';}await store.mutate(table,'delete',record);render();toast(`${batch?'Batch':'Trainee'} deleted.`);});}
function addProfileDeleteButton(trainee){if(!trainee||!store.canDelete('trainees')||trainee.source_id)return;const actions=document.querySelector('.modal-actions');if(!actions)return;const button=document.createElement('button'),label=document.createElement('span');button.type='button';button.className='btn danger';button.dataset.action='delete-trainee';button.dataset.id=trainee.id;label.textContent='Delete trainee';button.append(label);actions.append(button);}
...
document.addEventListener('click',async event=>{const el=event.target.closest('[data-action]');if(!el||el.disabled)return;const action=el.dataset.action,id=el.dataset.id;if(!store.user&&!['login','retry-connection','theme','nav'].includes(action))return;try{switch(action){
 ...
 case 'delete-trainee':deleteOperationalRecord('trainees',store.data.trainees.find(t=>t.id===id));break;
 case 'profile':{const trainee=store.data.trainees.find(t=>t.id===id);traineeProfile(ctx,trainee);addProfileDeleteButton(trainee);break;}
 ...
 case 'polish-comment':{if(!store.aiPolishEnabled){toast('AI comment polishing is not configured for this workspace.','error');break;}if(!document.getElementById('comment-ai-consent')?.checked){toast('Confirm before sending the comment to Gemini.','error');break;}const form=el.closest('form'),comment=form?.elements.instructor_comment?.value?.trim();if(!comment){toast('Write a comment before asking AI to polish it.','error');break;}el.disabled=true;el.innerHTML=icon('refresh',15)+' Polishing...';try{const result=await store.api('ai','POST',{kind:'polish-comment',comment,consent:true});form.elements.instructor_comment.value=result.comment;toast('Polished draft added. Review it before saving.');}catch(err){toast(err.message,'error');}finally{if(el.isConnected){el.disabled=false;el.innerHTML=icon('spark',16)+'<span>Polish comment with AI</span>';}}break;}
 ...
 case 'reset-filters':ctx.batchId='';ctx.companyId='';ctx.search='';ctx.traineeStatus='';ctx.page=1;ctx.selection.clear();ensureContext();render();break;
