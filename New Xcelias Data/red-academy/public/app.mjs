...
try{if(localStorage.getItem('red-academy-theme')==='light')document.body.classList.add('light');}catch{}
...
function render(){
 if(store.status==='loading'){document.getElementById('app').innerHTML='<div class="boot"><span class="brand-mark">R</span><p>Verifying your RED workspace...</p></div>';return;}
 ...
 document.title=`${auth?store.setupRequired?'Set up your workspace':'Sign in':NAV.find(n=>n[0]===ctx.route)?.[1]||'Workspace settings'} | RED Academy`;
...
function bindAuth(){
 ...
 try{
  ...
  }else{await store.login(values.get('email'),values.get('password'));ctx.loginEmail='';go('dashboard');toast('Welcome to RED Academy.');}
...
document.addEventListener('click',async event=>{const el=event.target.closest('[data-action]');if(!el||el.disabled)return;const action=el.dataset.action,id=el.dataset.id;if(!store.user&&!['login','retry-connection','theme','nav'].includes(action))return;try{switch(action){
 ...
 case 'theme':document.body.classList.toggle('light');try{localStorage.setItem('red-academy-theme',document.body.classList.contains('light')?'light':'dark');}catch{}render();break;
