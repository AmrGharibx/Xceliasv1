...
with tempfile.TemporaryDirectory(prefix='red-private-browser-') as temporary:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            check('Dark theme is active before changing the signature theme',page.evaluate('!document.body.classList.contains("light")'))
            check('Dark X Gh uses polished silver-red contrast',page.evaluate('getComputedStyle(document.querySelector(".academy-brand-signature")).color==="rgb(229, 235, 244)"'))
            check('Dark X Gh signature remains animated',page.evaluate('getComputedStyle(document.querySelector(".academy-brand-signature")).animationName==="academy-signature-live"'))
            click(page,'theme')
            ...
            check('Sidebar and header show visible star overlays, glass blur, and an animated signature sparkle',page.evaluate('Number(getComputedStyle(document.querySelector(".sidebar"),"::before").opacity)>0.3 && Number(getComputedStyle(document.querySelector(".topbar"),"::before").opacity)>0.3 && getComputedStyle(document.querySelector(".academy-brand-signature"),"::after").animationName==="academy-signature-spark" && getComputedStyle(document.querySelector(".sidebar")).backgroundImage.includes("linear-gradient")'))
            check('Light navigation has clearer glass, denser moving stars, and a refined red signature',page.evaluate('(()=>{const side=getComputedStyle(document.querySelector(".sidebar")),top=getComputedStyle(document.querySelector(".topbar")),stars=getComputedStyle(document.querySelector(".sidebar"),"::before"),headerStars=getComputedStyle(document.querySelector(".topbar"),"::before"),signature=getComputedStyle(document.querySelector(".academy-brand-signature"));return side.backgroundImage.includes("0.58")&&top.backgroundImage.includes("0.56")&&(stars.backgroundImage.match(/radial-gradient/g)||[]).length>=14&&(headerStars.backgroundImage.match(/radial-gradient/g)||[]).length>=14&&Number(stars.opacity)>0.7&&Number(headerStars.opacity)>0.7&&signature.color==="rgb(143, 31, 43)"})()'))
            ...
            invite=owner.call('invitations','POST',{'email':'name-qa@example.test','full_name':'QA Staff Before','role':'viewer'})
            check('Admin can create a disposable invited team account for the name editor test',invite['status']==201)
            invitation=json.loads(invite['text'])
            activation=h.Transport().call('auth/accept-invitation','POST',{'token':invitation['token'],'password':'Name-QA-Password-123!'})
            check('Invited QA account activates with its assigned role',activation['status']==201)
            go(page,'settings');click(page,'users')
            qa_row=page.locator('[data-user-row]').filter(has_text='name-qa@example.test')
            qa_row.locator('[data-user-name]').fill('QA Staff Renamed')
            qa_row.locator('[data-action="save-user-name"]').click()
            expect(page.locator('[data-user-row]').filter(has_text='name-qa@example.test').locator('[data-user-name]')).to_have_value('QA Staff Renamed')
            saved_user=next(user for user in json.loads(owner.call('users')['text']) if user['email']=='name-qa@example.test')
            check('Admin can edit a staff display name without changing role or active access',saved_user['full_name']=='QA Staff Renamed' and saved_user['role']=='viewer' and saved_user['active'])
            ...
            go(page,'batches')
            expect(page.locator('main')).to_contain_text(batch['batch_name'])
            page.evaluate('(id)=>__redTest.ctx.go("batches",{detailId:id})',batch['id'])
            click(page,'archive-batch')
            expect(page.locator('[data-action="confirm"]')).to_contain_text('Archive batch')
            click(page,'confirm')
            page.wait_for_function('__redTest.store.data.batches[0].archived_at!==null')
            expect(page.locator('h1')).to_have_text('Archived batches')
            expect(page.locator('main')).to_contain_text(batch['batch_name'])
            check('Archiving hides a batch from the regular board and retains it in Archived batches',state(owner)['batches'][0]['archived_at'] is not None and len(state(owner)['trainees'])==1)
            click(page,'batch-detail')
            expect(page.locator('h1')).to_have_text(batch['batch_name'])
            expect(page.locator('[data-action="restore-batch"]')).to_be_visible()
            expect(page.locator('[data-action="edit-batch"]')).to_have_count(0)
            check('Archived batch detail is read-only and offers restore',True)
            click(page,'restore-batch')
            page.wait_for_function('__redTest.store.data.batches[0].archived_at===null')
            click(page,'back-batches')
            expect(page.locator('main')).not_to_contain_text(batch['batch_name'])
            click(page,'batch-archive-view')
            expect(page.locator('main')).to_contain_text(batch['batch_name'])
            check('Restoring preserves the batch and returns it to the regular batch list',state(owner)['batches'][0]['archived_at'] is None)
            go(page,'reports')
