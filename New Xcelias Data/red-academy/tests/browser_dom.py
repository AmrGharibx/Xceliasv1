...
with tempfile.TemporaryDirectory(prefix='red-private-browser-') as temporary:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            check('Light mode shows the transparent academy logo, animated signature, kept-hidden shortcuts, and reachable Reports navigation',page.evaluate('document.body.classList.contains("light") && document.querySelector(".academy-brand-logo")?.alt==="Red Training Academy" && getComputedStyle(document.querySelector(".academy-brand-logo")).display==="block" && getComputedStyle(document.querySelector(".academy-brand-logo")).filter.includes("brightness") && getComputedStyle(document.querySelector(".academy-brand")).backgroundColor==="rgba(0, 0, 0, 0)" && document.querySelector(".academy-brand-signature")?.textContent.trim()==="X Gh" && getComputedStyle(document.querySelector(".academy-brand-signature")).animationName==="academy-signature-live" && getComputedStyle(document.querySelector(".sidebar-note")).display==="none" && getComputedStyle(document.querySelector(".workspace-switch")).display==="none" && getComputedStyle(Array.from(document.querySelectorAll(".sidebar .nav-link")).find(link=>link.getAttribute("href")==="#/migration")).display==="none" && Boolean(Array.from(document.querySelectorAll(".nav-link")).some(link=>link.getAttribute("href")==="#/analytics")) && Boolean(Array.from(document.querySelectorAll(".nav-link")).some(link=>link.getAttribute("href")==="#/reports")) && getComputedStyle(document.querySelector("canvas.starfield")).mixBlendMode==="multiply" && getComputedStyle(document.querySelector(".sidebar")).backdropFilter.includes("blur")'))
            ...
            for route in ['dashboard','batches','trainees','attendance','summaries','assessments','reports','companies','analytics','settings']:
                go(page,route)
            ...
            expect(page.locator('#reports-batch-filter')).to_be_visible()
            expect(page.locator('#reports-company-filter')).to_be_visible()
            check('Reports tab offers batch and company scope controls without replacing batch reports')
            page.locator('#reports-batch-filter').select_option(batch['id'])
            page.locator('#reports-company-filter').select_option(data['trainees'][0]['company_id'])
            expect(page.locator('.reports-table tbody tr')).to_have_count(1)
            expect(page.locator('.reports-table tbody tr')).to_contain_text(batch['batch_name'])
            check('Report center filters the selected batch and company using live academy records')
            page.locator('.reports-table [data-action="batch-report"]').click()
            expect(page.locator('#academy-report-builder')).to_be_visible()
            expect(page.locator('#academy-report-company')).to_have_value(data['trainees'][0]['company_id'])
            expect(page.locator('[data-academy-company-pdf]')).to_be_visible()
            expect(page.locator('[data-academy-all-pdfs]')).to_be_visible()
            expect(page.locator('[data-academy-build-report]')).to_be_visible()
            check('Reports tab opens the original PDF/photo/preview builder with company preselected')
            click(page,'close-modal')
            go(page,'migration')
            expect(page.locator('h1')).to_have_text('Notion import & review')
            check('Hidden Notion sidebar shortcut keeps its route and functionality')
